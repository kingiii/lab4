...
fun equalsTyVarPrefix ({ty = ty1, id = id1, prefix = p1}, 
                      ...
                      = id1
                      = id2 andalso Tipe.equals (ty1, ty2)
...
fun greaterVar({ty = ty1 : Tipe.t, id = name1 : string, prefix = prefix1 : string},
		       {ty = ty2 : Tipe.t, id = name2 : string, prefix = prefix2 : string})
              = name1 > name2
...
fun qSortClass (class: Class.t) =
    case class of
        ...
        let
          val newMethods = let
                              fun quicksortMethod method = 
                                  let
                                    fun qsort [] = []
                                      | qsort [x] = [x]
                                      | qsort (p::method) = 
                                        let
                                          val greater = fn x => Method.greater(x, p)
                                        in
                                          qsort (filt (not o greater) method) @ p :: (qsort (filt greater method))
                                        end
                                  in
                                    qsort method
                                  end
                            in
                              quicksortMethod methods1
                           end
          val newVars = let
                          fun quicksortVar var = 
                              let
                                  fun qsort [] = []
                                    | qsort [x] = [x]
                                    | qsort (p::var) = 
                                      let
                                        val greater = fn x => Class.greaterVar(x, p)
                                      in
                                        qsort (filt (not o greater) var) @ p :: (qsort (filt greater var))
                                      end
                              in
                                qsort var
                              end      
...
and qSortClasses c = List.map qSortClass c
...
fun sortProg f = 
    case f
      of T {mainClass   = main,
            ...
            pos         = pos1} => 
            let 
              val newClass = qSortClasses class1
            in
              T{mainClass   = main,
                classes     = newClass,
                pos         = pos1}
